...
import static com.peergine.android.conference.pgLibConference.EVENT_VIDEO_JOIN_1;
...
public class ComplexFragment extends SupportFragment {
    ...
    class MEMBER {
        ...
        SurfaceView pViewL = null;
        LinearLayout pLayoutL = null;
    }
    ...
    void initView(final View view) {
        ...
        oMemb.pLayout = view.findViewById(R.id.layoutVideoS2);
        oMemb.pLayout.setOnClickListener(layoutOnClick);
        oMemb.pLayoutL = view.findViewById(R.id.layoutVideoS3);
        oMemb.pLayoutL.setOnClickListener(layoutOnClick);
...
//---------------------------------------------------------------------
    ...
    private void EventVideoOpenL(String sAct, String sData, String sPeer) {
        //收到视频请求
        showInfo(sPeer + " 请求视频连线->同意L");
        //// TODO: 2016/11/7 在这之后回复
        //调用
        pgVideoOpen(sPeer,true);
    }
    ...
    private void EventVideoJoin(String sAct, String sData, String sPeer) {
        ...
        if ("0".equals(sData)) {
            ...
            Log.d("", sPeer + " 打开失败");
            pgVideoRestore(sPeer,false);
        }
    }

    private void EventVideoJoinL(String sAct, String sData, String sPeer) {
        // TODO: 2016/11/8 请求端会收到请求打开视频的结果，打开视频成功除了显示和播放视频外，还有这个事件
        if ("0".equals(sData)) {
            showInfo(sPeer + ":" + "视频成功打开L");
            Log.d("", sPeer + " 成功打开L");
        } else {
            showInfo(sPeer + ":" + "视频成功失败L");
            Log.d("", sPeer + " 打开失败L");
            pgVideoRestore(sPeer,true);
        }
    }
    ...
    private void EventVideoCloseL(String sAct, String sData, String sPeer) {
        // TODO: 2016/11/8  通知应用程序视频已经挂断
        showInfo(sPeer + " 已经挂断视频L");
        pgVideoRestore(sPeer,true);
    ...
    private OnEventListener m_OnEvent = new OnEventListener() {
        ...
        public void event(String sAct, String sData, final String sPeer) {
            ...
            } else if (sAct.equals(EVENT_VIDEO_OPEN_1)) {
                EventVideoOpenL(sAct, sData, sObjPeer);
            ...
            } else if (sAct.equals(EVENT_VIDEO_CLOSE_1)) {
                EventVideoCloseL(sAct, sData, sObjPeer);
            } else if (sAct.equals(EVENT_VIDEO_JOIN)) {
                EventVideoJoin(sAct, sData, sObjPeer);
            } else if (sAct.equals(EVENT_VIDEO_JOIN_1)) {
                EventVideoJoinL(sAct, sData, sObjPeer);
...
//        }
    ...
    private void pgStop() {
        ...
        for (int i = 0; i < mListMemberS.size(); i++) {
            ...
            if (!"".equals(oMemb.sPeer)) {
                pgVideoClose(oMemb.sPeer,true);
                pgVideoClose(oMemb.sPeer,false);
            }
        }
        mConf.AudioStop();
        mConf.VideoStop();
     ...
     * @return
     */
    private boolean pgVideoOpen(String sPeer,boolean bLarge) {
        ...
        if(!bLarge) {
            ...
            MembTmp.pViewL = mConf.VideoOpenL(sPeer, 160, 120);
            if (MembTmp.pViewL != null) {
                MembTmp.pLayoutL.removeAllViews();
                MembTmp.pLayoutL.addView(MembTmp.pViewL);
            }
        }
     ...
     * @param sPeer 重置节点
     */
    private void pgVideoRestore(String sPeer,boolean bLarge) {
        MEMBER membTmp = memberSearch(sPeer);
        if (membTmp != null) {
            if (membTmp.sPeer.equals(sPeer)) {
                if(!bLarge) {
                    ...
                    membTmp.pLayoutL.removeView(membTmp.pView);
                    membTmp.pViewL = null;
                }
                if( membTmp.pView == null && membTmp.pViewL==null){
                    membTmp.sPeer = "";
                }
            }
        }
    ...
    //清理窗口数据和关闭视频
    private void pgVideoClose(String sPeer,boolean bLarge) {
        if(!bLarge) {
            mConf.VideoClose(sPeer);
        }else{
            mConf.VideoCloseL(sPeer);
        }
        pgVideoRestore(sPeer,bLarge);
    }//清理窗口数据和关闭视频
